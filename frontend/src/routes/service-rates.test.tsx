...
function baseFetch(ratePost?: () => Response) {
  return vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes('/api/v1/services/10/package-mappings?')) return response(page([packageItem]))
    if (url.includes('/api/v1/services/11/package-mappings?')) return response(page([]))
    if (url.includes('/api/v1/services?')) return response(page(services))
    if (url.endsWith('/api/v1/services/10/packages/20/rates') && init?.method === 'POST') return ratePost?.() ?? response(rate, 201)
    if (url.endsWith('/api/v1/services/10/packages/20/rates/40') && init?.method === 'PUT') return response({ ...rate, unit_rate: '8000.00', is_active: false, is_available: false })
    if (url.includes('/api/v1/rates?')) return response(page([rate]))
...
test('edits and deactivates a service rate', async () => {
  ...
  await waitFor(() => expect(fetchMock.mock.calls.filter(([url, init]) => String(url).endsWith('/api/v1/services/10/packages/20/rates/40') && init?.method === 'PUT').length).toBeGreaterThanOrEqual(2))
})
