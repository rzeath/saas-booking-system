  ...
  return (
    <section>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-2">
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6"><h2 className="text-lg font-semibold">Event</h2><dl className="mt-5 grid gap-5 sm:grid-cols-2"><Detail label="Event date" value={formatBusinessDate(item.event_date)} /><Detail label="Start time" value={formatManilaTime(item.start_time)} /><Detail label="Effective end" value={formatManilaScheduleEnd(item.end_at, item.event_date)} /><Detail label="Event type snapshot" value={item.event_type_snapshot.name} /><Detail label="Venue" value={item.venue_name} /><div className="sm:col-span-2"><Detail label="Venue address" value={item.venue_address} /></div><Detail label="Contact person" value={item.contact_person} /><Detail label="Contact number" value={item.contact_number} /></dl></div>
      ...
      <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900"><div className="border-b border-slate-800 p-6"><h2 className="text-lg font-semibold">Scheduled services</h2><p className="mt-1 text-sm text-slate-400">All services begin at the shared event start; each duration determines its effective end.</p></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-slate-800 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Service / package</th><th className="px-5 py-3">Effective interval</th><th className="px-5 py-3">Duration</th><th className="px-5 py-3">Quantity</th><th className="px-5 py-3">Staff</th><th className="px-5 py-3">Unit rate</th><th className="px-5 py-3 text-right">Line total</th></tr></thead><tbody className="divide-y divide-slate-800">{item.booking_services.map((line) => <tr key={line.id}><td className="px-5 py-4"><strong className="block">{line.service.name}</strong><span className="text-slate-400">{line.package.name}</span></td><td className="px-5 py-4"><span className="whitespace-nowrap">{formatManilaTime(line.start_at)} – {formatManilaScheduleEnd(line.end_at, line.start_at.split(' ')[0] ?? '')}</span></td><td className="px-5 py-4">{durationLabel(line.duration_minutes)}</td><td className="px-5 py-4">{line.quantity}</td><td className="px-5 py-4">{line.staff.length > 0 ? line.staff.map((staff) => staff.name).join(', ') : <span className="text-slate-500">Unassigned</span>}</td><td className="px-5 py-4">{formatMoney(line.unit_rate)}</td><td className="px-5 py-4 text-right font-medium">{formatMoney(line.line_total)}</td></tr>)}</tbody><tfoot className="border-t border-slate-700"><tr><th colSpan={6} className="px-5 py-4 text-right">Services total</th><td className="px-5 py-4 text-right text-lg font-semibold">{formatMoney(total)}</td></tr></tfoot></table></div></div>
      <QuotationHistory booking={item} />
